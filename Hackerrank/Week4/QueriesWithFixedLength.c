...
int* solve(int arr_count, int* arr, int queries_count, int* queries, int* result_count) {
    int n = arr_count, q = queries_count;
    *result_count = q;
    int *res = malloc(q * sizeof(int));
    if (n == 0) { for (int i = 0; i < q; i++) res[i] = 0; return res; }
    ...
    for (int i = 0; i < n; i++) {
        ...
        prev[i] = (top >= 0) ? st[top] : -1;
        st[++top] = i;
    }
    top = -1;
    for (int i = n - 1; i >= 0; i--) {
        while (top >= 0 && B[st[top]] > B[i]) top--;
        next[i] = (top >= 0) ? st[top] : n;
        st[++top] = i;
    }
    int *best = malloc((n + 1) * sizeof(int));
    for (int w = 1; w <= n; w++) best[w] = INT_MIN;
    for (int i = 0; i < n; i++) {
        int len = next[i] - prev[i] - 1; 
        if (B[i] > best[len]) best[len] = B[i];
    }
    for (int w = n - 1; w >= 1; w--) {
    ...
    free(B); free(prev); free(next); free(st); free(best);
    return res;
}
