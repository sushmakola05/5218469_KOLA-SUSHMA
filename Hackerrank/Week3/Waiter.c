 ...
 *
 */
 static void getPrimes(int *pr, int q){
    bool mark[10001] = {0};
    int k = 0;
    for(int i = 2; k < q; i++){
        if(!mark[i]){
            pr[k++] = i;
            for(int j = i + i; j <= 10000; j+= i) mark[j] = 1;
        }
    }
 }
    ...
    for(int i = 0; i < q; i++){
        ...
        for(int j = number_count - 1; j >= 0; j--){
            if(number[j] % primes[i] == 0)
                ...
                next[nextCount++] = number[j];
        }
        for(int j = biCount - 1; j >= 0; j--)
            res[idx++] = bi[j];
        free(bi);
        free(number);
        number = next;
        number_count = nextCount;
    }
    for(int j = number_count - 1; j >= 0; j--) res[idx++] = number[j];
    *result_count = idx;
    return res;
}
